...
| `type` | `"idea" \| "knowledge" \| "thread" \| "review" \| "plan"` | 否 | `schema` | 仅创建时生成，后续不改 |
...
| `origin.workspace_ref` | `string` | 是 | `capture` | 来源具备明确工作区/目录身份时生成；后续不改 |
...
- `type` 一旦创建不再变更，与 `id` 同等不可变。
- 如果 piece 的性质确实发生了根本变化，不应修改原 piece 的 `type`，而应采用以下方式之一：
  - 删除原 piece，新建一个正确 `type` 的 piece（走正常入库流程）。
  - 保留原 piece 不变，另新建一个不同 `type` 的 piece（走正常入库流程），新旧 piece 之间通过 `links` 关联。
- 这样做的原因是：`type` 决定了物理存储路径（`pieces/<type>/<year>/`），如果允许变更 `type` 则需要移动目录，引入原子性和索引一致性风险。
...
它和 `origin.workspace_ref` 不同：

- `origin.workspace_ref` 表示这条内容来自哪个工作区或目录身份
- `projects` 表示这条内容在认知上属于哪些长期项目
...
> **设计决策**：`catalog` 层不再存储 `domains`、`methods`、`projects`、`contexts`、`status` 的镜像字段。这些信息在运行时直接从 `meta.json` 读取。原因：（1）避免写入时的一致性维护负担；（2）消除 meta 与 surface 之间数据不一致的风险；（3）catalog 层的价值应在于 AI 精炼的轻量判断（`summary` + `salience`），而非机械复制 meta 字段。
