...
长期上，它还可以成为普通问答的后台知识与记忆底座，但第一版不默认自动调用。
...
至于外部会话偏好、回答策略和长期互动调节，这些都不在这份 IPK 文档里展开。
...
为了实现你说的“全知全能又很懂我的助理”，未来普通问答可以接一条轻量 IPK 路由过程。

这个过程不是每次都重检全库，而是：

1. 先判断问题是否值得查 IPK
2. 若系统有额外的会话上下文层，可先读取其输出作为附加上下文
3. 再选择检索路径
...
未来普通问答如果需要更贴合当前会话，可以在 IPK 路由前接入额外上下文层。
