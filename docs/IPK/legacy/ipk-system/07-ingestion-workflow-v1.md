...
- 用户点击“一键生成 piece 并入库”之后，系统到底应该怎么跑
- 哪些内容由 AI 自动生成
- 哪些内容需要用户确认
- 哪些阶段需要重写
- 什么时候真正写盘
...
```text
docs/IPK/pieces/idea/2026/piece_20260403_001/
```
...
- 只有 `accept` 后才真正写盘
- `discard` 不写盘
- `revise` 不写盘，只回到 `draft` / `structure` / `quality`
...
IPK 的“一键生成 piece 并入库”本质上不是一个保存动作，而是一条：
